 ...
 */
// @ts-ignore
exports.daydiff = function (first, second) { return Math.round(second - first); };
/**
 ...
 */
// @ts-ignore
exports.zip = function (rows) { return rows[0].map(function (_, c) { return rows.map(function (row) { return row[c]; }); }); };
/**
...
exports.cummulativeSeperation = function (dates, labelWidth, minEventPadding, maxEventPadding, startPadding) {
    ...
    for (var index = 1; index < distances.length; index += 1) {
        var distance = exports.daydiff(dates[index - 1], dates[index]);
